...
class QuillSystemContextMenu extends StatefulWidget {
  ...
  factory QuillSystemContextMenu.quillEditor({
    ...
    final points =
        quillEditorState.renderEditor.getEndpointsForSelection(selection);
    ...
    if (selection.isValid && !selection.isCollapsed) {
      final startCharacterRect =
          quillEditorState.renderEditor.getLocalRectForCaret(selection.base);
      final endCharacterRect =
          quillEditorState.renderEditor.getLocalRectForCaret(selection.extent);
      startGlyphHeight = startCharacterRect.height;
      endGlyphHeight = endCharacterRect.height;
    } else {
      startGlyphHeight =
          quillEditorState.renderEditor.preferredLineHeight(selection.base);
  ...
  static List<IOSSystemContextMenuItem> getDefaultItemsForQuill(
      QuillRawEditorState quillEditorState) {
    return <IOSSystemContextMenuItem>[
      ...
      if (quillEditorState.selectAllEnabled)
        const IOSSystemContextMenuItemSelectAll(),
      if (quillEditorState.lookUpEnabled)
        const IOSSystemContextMenuItemLookUp(),
      if (quillEditorState.searchWebEnabled)
        const IOSSystemContextMenuItemSearchWeb(),
    ];
  }
